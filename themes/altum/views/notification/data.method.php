    ...
    <div class="d-flex flex-column flex-md-row justify-content-between">
        <div class="d-flex">
            ...
            <div>
                <?php if(count($data->conversions)): ?>
                    <a href="<?= url('notification/' . $data->notification->notification_id . '/data/' . $data->date->start_date . '/' . $data->date->end_date . '?page=' . ($_GET['page'] ?? 1) . '&json') ?>" target="_blank" class="btn btn-sm btn-light rounded-pill mr-3"><i class="fa fa-fw fa-file-csv"></i> <?= $this->language->global->export_json ?></a>
                <?php endif ?>
        ...
        <div>
            <button
                    ...
                    data-min-date="<?= (new \DateTime($data->notification->date))->format('Y-m-d') ?>"
...
<script>
    ...
    $('#daterangepicker').daterangepicker({
        ...
        /* Redirect */
        redirect(`${$('#base_controller_url').val()}/data/${start.format('YYYY-MM-DD')}/${end.format('YYYY-MM-DD')}`, true);
