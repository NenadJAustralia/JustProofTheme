...
/* Logs for the charts */
$result = \Altum\Database\Database::$database->query("
    SELECT
         `type`,
         COUNT(`id`) AS `total`
    FROM
...
<div class="table-responsive table-custom-container">
    <table class="table table-custom">
        ...
        <?php while($row = $result->fetch_object()): ?>
            <tr>
                <td>
                    <i class="<?= $this->language->notification->score_feedback->icon ?>"></i> <?= $this->language->notification->score_feedback->{$row->type} ?>
                </td>
                <td><?= nr($row->total) ?></td>
            </tr>
        <?php endwhile ?>
...
<script>
    ...
    new Chart(clicks_chart, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: <?= json_encode($this->language->notification->score_feedback->feedback_score_1) ?>,
                    data: <?= $data->logs_chart['feedback_score_1'] ?? '[]' ?>,
                    borderColor: '#ED4956',
                    fill: false
                },
                {
                    label: <?= json_encode($this->language->notification->score_feedback->feedback_score_2) ?>,
                    data: <?= $data->logs_chart['feedback_score_2'] ?? '[]' ?>,
                    borderColor: '#ed804c',
                    fill: false
                },
                {
                    label: <?= json_encode($this->language->notification->score_feedback->feedback_score_3) ?>,
                    data: <?= $data->logs_chart['feedback_score_3'] ?? '[]' ?>,
                    borderColor: '#8f8f8f',
                    fill: false
                },
                {
                    label: <?= json_encode($this->language->notification->score_feedback->feedback_score_4) ?>,
                    data: <?= $data->logs_chart['feedback_score_4'] ?? '[]' ?>,
                    borderColor: '#6c94ed',
                    fill: false
                },
                {
                    label: <?= json_encode($this->language->notification->score_feedback->feedback_score_5) ?>,
