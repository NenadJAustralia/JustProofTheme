...
<div class="mb-5 row justify-content-between">
    <div class="col-12 col-sm-6 col-xl-3 mb-4">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body">
                <small class="text-muted"><i class="fa fa-fw fa-sm fa-bell mr-1"></i> <?= $this->language->admin_index->display->current_month_notifications_impressions ?></small>

                <div class="mt-3"><span class="h4"><?= nr($data->notifications->current_month_notifications_impressions) ?></span></div>
            </div>
        </div>
    </div>
    ...
    <div class="col-12 col-sm-6 col-xl-3 mb-4">
        <div class="card d-flex flex-row h-100 overflow-hidden">
            <div class="card-body">
                <small class="text-muted"><i class="fa fa-fw fa-sm fa-bell mr-1"></i> <?= $this->language->admin_index->display->total_notifications_impressions ?></small>

                <div class="mt-3"><span class="h4"><?= nr($data->total_track_notifications) ?></span></div>
            </div>
        </div>
    </div>
...
<?php $result = \Altum\Database\Database::$database->query("SELECT `campaigns`.*, `users`.`name` AS `user_name` FROM `campaigns` LEFT JOIN `users` ON `campaigns`.`user_id` = `users`.`user_id` ORDER BY `campaign_id` DESC LIMIT 5"); ?>
<?php if($result->num_rows): ?>
    <div class="mb-5">
        <h1 class="h3 mb-4"><?= $this->language->admin_index->campaigns->header ?></h1>

        <div class="table-responsive table-custom-container">
            <table class="table table-custom">
                <thead>
                <tr>
                    <th><?= $this->language->admin_index->campaigns->campaign ?></th>
                    <th><?= $this->language->admin_index->campaigns->user ?></th>
                    <th></th>
                    <th></th>
                </tr>
                </thead>
                ...
                <?php while($row = $result->fetch_object()): ?>
                    <tr>
                        ...
                        <td>
                            <div class="d-flex flex-column">
                                <div><?= $row->is_enabled ? '<span class="badge badge-pill badge-success"><i class="fa fa-fw fa-check"></i> ' . $this->language->global->active . '</span>' : '<span class="badge badge-pill badge-warning"><i class="fa fa-fw fa-eye-slash"></i> ' . $this->language->global->disabled . '</span>' ?></div>
                                <div><small class="text-muted" data-toggle="tooltip" title="<?= \Altum\Date::get($row->date, 1) ?>"><?= \Altum\Date::get($row->date, 2) ?></small></div>
                            </div>

                        <td><?= ''; //get_admin_options_button('campaign', $row->campaign_id) ?></td>
                    </tr>
                <?php endwhile ?>
                </tbody>
            </table>
